...
describe('verifyJWT()', () => {
  ...
  const badJwt =
    'eyJ0eXAiOiJKV1QiLCJhbGciOiJFUzI1NksifQ.eyJpc3MiOiJkaWQ6dXBvcnQ6Mm5RdGlRRzZDZ20xR1lUQmFhS0Fncjc2dVk3aVNleFVrcVgiLCJpYXQiOjE0ODUzMjExMzMsInJlcXVlc3RlZCI6WyJuYW1lIiwicGhvbmUiXX0.1hyeUGRBb-cgvjD5KKbpVJBF4TfDjYxrI8SWRJ-GyrJrNLAxt4MutKMFQyF1k_YkxbVozGJ_4XmgZqNaW4OvCX'
  it('rejects a JWT with bad signature', () => {
    return verifyJWT(badJwt)
      .catch(error => expect(error.message).toEqual('Signature invalid for JWT'))
  ...
  it('rejects an expired JWT', () => {
    return createJWT({ exp: NOW - IAT_SKEW - 1 }, { issuer: did, signer }).then(jwt =>
      verifyJWT(jwt)
        .catch(error => expect(error.message).toEqual('JWT has expired: exp: 1485320832 < now: 1485321133'))
        .then(p => expect(p).toBeFalsy()),
    )
  })
  ...
  describe('DID', () => {
    ...
    it('errors if no suitable public keys exist', async () => {
      ...
      return expect(resolveAuthenticator(alg, did)).rejects.toEqual(
        new Error(`DID document for ${did} does not have public keys for ${alg}`),
      )
    })
